...
<html:html>
	...
	<script language="javascript">
		function extraActions( action )
		{
			if ( action == 'insertDomain' ) {
				...
				document.forms[0].action = "BkrPortfMgmt.BkrPortfMgmtMovementDetail.Update.DeleteDomain"; 
				document.forms[0].backURL.disabled = true; 
			}

			else if (action =='listBroker')
			{
				document.forms[0].action = "BkrPortfMgmt.BkrPortfMgmtMovementDetail.Update.List";
				document.forms[0].backURL.disabled = false;
			}

			else if (action =='listBkrPortfMgmt')
			{
				document.forms[0].action = "BkrPortfMgmt.BkrPortfMgmtMovementDetail.Update.ListBkrPortfMgmt";
				document.forms[0].backURL.disabled = false;
			}
	
			else if(action== 'clearPage')
			{
				document.forms[0].action = "BkrPortfMgmt.BkrPortfMgmtMovementDetail.Update.Update";
				document.forms[0].backURL.disabled = false;	
			}
		};
	...
	<jsp:include page="/View/Util/JavascriptMasks.jsp" flush="true"></jsp:include>
		...
		<TITLE> Alteração de Corretora x Carteira com Pendência de Aprovação </TITLE>
	</HEAD>

	<body>
		<html:form action="/BkrPortfMgmt.BkrPortfMgmtMovementDetail.Update.Show.do">
				...
				<tr>
					...
					<td>
						<table class="ODS_internalWidth" border="0">
							<thead>
								<tr>
									<th class="subtitle" scope="colgroup" colspan="5">Alteração de Corretora x Carteira com Pendência de Aprovação</th>
								</tr>
							</thead>
							...
							<tr>
								...
								<table id="gridTable" width="1200px" border="0">
									<tbody>
										...
										<tr class="fixed">
										<%int rowIndex = 0;%>
										<logic:notEmpty name="BkrPortfMgmtMovementDetailForm"	property="brokerGrid">
											<logic:iterate name="BkrPortfMgmtMovementDetailForm"
												property="brokerGrid" indexId="index" id="row">
												<%
												BkrPortfMgmtMovementDetailForm bkrPortfMgmtDetailForm = (BkrPortfMgmtMovementDetailForm) session.getAttribute("BkrPortfMgmtMovementDetailForm");
												...
												<bean:define name="BkrPortfMgmtMovementDetailForm" property='<%="selectedItemsInBrokerGrid["+index+"]"%>' id="selectedItemsInBrokerGrid" type="java.lang.String" />

												<td width="2%" align="center">

													<input type="checkbox" class="checkbox" name=<%="selectedItemsInBrokerGrid["+index+"]"%> value="S" <%=selectedItemsInBrokerGrid.equals("S")?"checked=\"checked\"":""%> onclick="javascript:enableAddBrokerButton()" />

												</td>
												<td class="alignRight" width="9%"><%=resultLine[BkrPortfMgmtMovementDetailForm.COL_POS_BKR_CNPJ_NBR]%></td>
												<td class="alignLeft" width="38%"><%=resultLine[BkrPortfMgmtMovementDetailForm.COL_POS_BKR_NAME_TEXT]%></td>
												<td class="alignLeft" width="51%"><%=resultLine[BkrPortfMgmtMovementDetailForm.COL_POS_BKR_ADDR_TEXT]%></td>
											</logic:iterate>
										</logic:notEmpty>
										</tr>
									</tbody>
					...
					<td>
						...
						<table border="0">
							...
							<tr>
								<td width="100%">
								<DIV class="ODS_DivGridVerticalScroll">
								<table id="gridTable" width="1500px" border="0">
									<tbody>
										<tr id="gridHeader" class="fixed">
											...
											<TH class="ODS_header" width="20%">Ação</TH>
										</tr>
										...
										<logic:notEmpty name="BkrPortfMgmtMovementDetailForm"	property="bkrCnpjNbrInBkrPortfMgmtGrid">
											<logic:iterate name="BkrPortfMgmtMovementDetailForm"
												...
												<bean:define name="BkrPortfMgmtMovementDetailForm" property='<%="updateSelectedItemsInBkrPortfMgmtGrid["+index+"]"%>' id="updateSelectedItemsInBkrPortfMgmtGrid" type="java.lang.String" />
												<bean:define name="BkrPortfMgmtMovementDetailForm" property='<%="deleteSelectedItemsInBkrPortfMgmtGrid["+index+"]"%>' id="deleteSelectedItemsInBkrPortfMgmtGrid" type="java.lang.String" />
												...
												<td class="alignRight" width="8%"><bean:write name="BkrPortfMgmtMovementDetailForm" property='<%="bkrCnpjNbrInBkrPortfMgmtGrid["+index+"]"%>'></bean:write></td>
												<td class="alignLeft" width="35%"><bean:write name="BkrPortfMgmtMovementDetailForm" property='<%="bkrNameTextInBkrPortfMgmtGrid["+index+"]"%>'></bean:write></td>
												...
												<td class="alignCenter" width="20%"><bean:write name="BkrPortfMgmtMovementDetailForm" property='<%="opernCodeInBkrPortfMgmtGrid["+index+"]"%>'></bean:write></td>
											</logic:iterate>
										</logic:notEmpty>
										</tr>
									</tbody>
								...
								<table class="ODS_internalWidth" border="0">
									<tbody>
										<TR>
											<TD width="100%"></TD>
												<ods:AccessGrantedTag m_SGFunction="/BkrPortfMgmt.BkrPortfMgmtDetail.Insert.Update">
													<TD><html:button property="insertBtn" value="Confirmar Alterações" onclick="submitAction('update');" disabled="true"></html:button></TD>
												</ods:AccessGrantedTag>
