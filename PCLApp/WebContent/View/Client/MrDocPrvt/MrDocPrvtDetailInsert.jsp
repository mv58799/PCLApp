...
<script language="javascript">
	function extraActions(action){
	    ...
	    if ( action == 'insertDomain' ) {
			document.forms[0].action = "MrDocPrvt.MrDocPrvtDetail.Insert.InsertDomain"; 
			document.forms[0].backURL.disabled = true; 
		}
	    else if ( action == 'deleteDomain' ) {
			document.forms[0].action = "MrDocPrvt.MrDocPrvtDetail.Insert.DeleteDomain"; 
			document.forms[0].backURL.disabled = true; 
		}
		...
		else if ( action == 'InsertConfirm' ) {			
			document.forms[0].isConfirmCancel.value = true;
			document.forms[0].action = "MrDocPrvt.MrDocPrvtDetail.Insert.Execute";
			document.forms[0].backURL.disabled = false;     
		}
		else if ( action == 'MrDocPrvt.MrDocPrvtList.List' ) {
			document.forms[0].action = "MrDocPrvt.MrDocPrvtList.List.Show.Clear"; 
			document.forms[0].backURL.disabled = false; 
		}
	...
	function showErrorMessage(){
		if (!verifyItemsSelected()){
			...
			validationMessageArea.innerHTML += 'Erro: O registro não pode ser inserido pois são obrigatórias uma ou mais associações com "Memo de Risco".<br>';
...
<TITLE>Inserção de Memo de Risco</TITLE>
</HEAD>
<body>

<html:form action="/MrDocPrvt.MrDocPrvtDetail.Insert.Show.do">
	...
	<table class="ODS_mainTable" cellpadding="0" cellspacing="0" border="0">
		<tr>
			<html:text property="backURL" value="MrDocPrvt.MrDocPrvtDetail.Insert.Show" style="display:none"></html:text>
			<html:text property="isMrActive" style="display:none"></html:text>
			<html:text property="isConfirmCancel" style="display:none"></html:text>
			
			<td>		
				<table class="ODS_internalWidth" border="0">
				<thead><tr><th class="subtitle" scope="colgroup" colspan="3">Inserção de Memo de Risco</th></tr></thead>
					<TBODY>
					 <tr><td>&nbsp;</td></tr>
						...
						<TR class="ODS_Detail_Line2">
						  <TD width="3%">
						    <html:text property="curAcctNbrSrc"	styleClass="ODS_Text_Field_Size_20" disabled="true"></html:text>
						  </TD>							
						  ...
						  <td colspan="2">
						    <html:text property="invstCurAcctNbrSrc"	styleClass="ODS_Text_Field_Size_10" disabled="true"></html:text>
								...
								<table class="ODS_internalWidth" border="0">
						<thead><tr><th class="subtitle" scope="colgroup" colspan="3">Dados de Callback</th>
						</tr></thead>
						<tr>
							<td width="100%">
	   ...
	   <logic:equal name="MrDocPrvtDetailForm" property="isMrActive" value="true">
	     <script language="javascript">
		      if (confirm('Já existe um Memo de Risco Ativo para essa conta, deseja cancelar o mesmo?')){			
			    submitAction('InsertConfirm');
			  }
