...
public class TplPrvtAuthPersnMovDAO extends BaseOracleDAO {
	...
	public ArrayList<AuthorizationPersonVO> list(String filterNumberEM, String filterCpfCnpj, String filterName, boolean isLike) throws ParseException {
		...
		try {
			...
			query.append(" FROM " + C_PL_SCHEMA + "TPL_PRVT_AUTH_PERSN_MOV ");
			query.append(" WHERE 1=1 ");
			
			if (filterNumberEM != null && !filterNumberEM.equals("")) {
				if(isLike){
					query.append(" AND UPPER(TPL_PRVT_AUTH_PERSN_MOV.EM_NBR) LIKE TRIM(?) ");
				}else{
					query.append(" AND UPPER(TPL_PRVT_AUTH_PERSN_MOV.EM_NBR) = TRIM(?) ");
				}				
			}
			
			if (filterCpfCnpj != null && !filterCpfCnpj.equals("")) {
				if(isLike){
					query.append(" AND UPPER(TPL_PRVT_AUTH_PERSN_MOV.CPF_CNPJ_NBR) LIKE TRIM(?) ");
				}else{
					query.append(" AND UPPER(TPL_PRVT_AUTH_PERSN_MOV.CPF_CNPJ_NBR) = TRIM(?) ");
				}
			}
			
			if (filterName != null && !filterName.equals("")) {
				if(isLike){
					query.append(" AND UPPER(TPL_PRVT_AUTH_PERSN_MOV.AUTH_PERSN_NAME) LIKE TRIM(?) ");
				}else{
					query.append(" AND UPPER(TPL_PRVT_AUTH_PERSN_MOV.AUTH_PERSN_NAME) = TRIM(?) ");
				}				
			}
	...
	public AuthorizationPersonVO insert(AuthorizationPersonVO vo) {
		...
		try {
			...
			sqlQuery.append("  INSERT INTO " + C_PL_SCHEMA + "TPL_PRVT_AUTH_PERSN_MOV ( " );
...
//			}
	...
	public AuthorizationPersonVO update(AuthorizationPersonVO vo) {
		...
		try {
			...
			sqlQuery.append("  UPDATE " + C_PL_SCHEMA + "TPL_PRVT_AUTH_PERSN_MOV SET " );
			sqlQuery.append(" AUTH_PERSN_NAME = ?, ");
			sqlQuery.append(" DOC_ID = ?, ");
			sqlQuery.append(" BIRTH_DATE = ?, ");
			sqlQuery.append(" CPF_CNPJ_NBR = ?, ");
			...
			sqlQuery.append(" ADDR_CNTRY_CODE = ?, ");
			sqlQuery.append(" ZIP_CODE = ? ");
			sqlQuery.append("WHERE TPL_PRVT_AUTH_PERSN_MOV.EM_NBR = ? ");
	...
	public void delete(String emNbr) {
		...
		try {
			...
			query.append(" DELETE FROM " + C_PL_SCHEMA + "TPL_PRVT_AUTH_PERSN_MOV WHERE EM_NBR = ? ");
