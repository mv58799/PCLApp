...
public class TplPrvtCpfStatDAO extends BaseOracleDAO {
	...
	public ArrayList<StatusCpfCnpjVO> list(String filterNumberEM, String filterCpfCnpj, String filterName, 
		...
		try {
			...
			query.append(" SELECT TPL_PRVT_CPF_STAT.CPF_CNPJ_NBR, ");
			query.append("   TPL_PRVT_CPF_STAT.CPF_STATUS, ");
			query.append("   TPL_PRVT_CPF_STAT.CPF_UPD_MTH_YR, ");
			query.append("   TPL_PRVT_CPF_STAT.LAST_AUTH_DATE, ");
			query.append("   TPL_PRVT_CPF_STAT.LAST_AUTH_USER_ID, ");
			query.append("   TPL_PRVT_CPF_STAT.LAST_UPD_DATE, ");
			query.append("   TPL_PRVT_CPF_STAT.LAST_UPD_USER_ID, ");
			query.append("   TPL_PRVT_CPF_STAT.REC_STAT_CODE ");
			query.append(" FROM " + C_PL_SCHEMA + "TPL_PRVT_CPF_STAT, " + C_PL_SCHEMA + "TPL_PRVT_CUSTOMER, " + C_PL_SCHEMA + "TPL_PRVT_AUTH_PERSN");
			query.append(" WHERE TPL_PRVT_CPF_STAT.CPF_CNPJ_NBR = TPL_PRVT_CUSTOMER.CUST_CPF_CNPJ_NBR (+) ");
			query.append(" AND TPL_PRVT_CPF_STAT.CPF_CNPJ_NBR = TPL_PRVT_AUTH_PERSN.CPF_CNPJ_NBR (+) ");
			...
			if (filterStatus != null && !filterStatus.equals("")) {
				if(isLike){
					query.append(" AND UPPER(TPL_PRVT_CPF_STAT.CPF_STATUS) LIKE TRIM(?) ");
				}else{
					query.append(" AND UPPER(TPL_PRVT_CPF_STAT.CPF_STATUS) = TRIM(?) ");
				}				
			}
			
			if (filterMonthYear != null && !filterMonthYear.equals("")) {
				if(isLike){
					query.append(" AND UPPER(TPL_PRVT_CPF_STAT.CPF_UPD_MTH_YR) LIKE TRIM(?) ");
				}else{
					query.append(" AND UPPER(TPL_PRVT_CPF_STAT.CPF_UPD_MTH_YR) = TRIM(?) ");
				}				
			}
			
			if (filterCpfCnpjList != null && !filterCpfCnpjList.equals("")) {
				query.append(" AND UPPER(TPL_PRVT_CPF_STAT.CPF_CNPJ_NBR) IN (?) ");	
			}
			
			query.append(" GROUP BY TPL_PRVT_CPF_STAT.CPF_CNPJ_NBR, ");
			query.append("   TPL_PRVT_CPF_STAT.CPF_STATUS, ");
			query.append("   TPL_PRVT_CPF_STAT.CPF_UPD_MTH_YR, ");
			query.append("   TPL_PRVT_CPF_STAT.LAST_AUTH_DATE, ");
			query.append("   TPL_PRVT_CPF_STAT.LAST_AUTH_USER_ID, ");
			...
			if (filterMonthYear != null && !StringUtils.isBlank(filterMonthYear)) {
				if(isLike){
					...
					preparedStatement.setString(count++, filterMonthYear.toUpperCase());
				}
			}
	...
	public StatusCpfCnpjVO insert(StatusCpfCnpjVO vo) {
		...
		try {
			...
			sqlQuery.append("  INSERT INTO " + C_PL_SCHEMA + "TPL_PRVT_CPF_STAT ( " );
	...
	public StatusCpfCnpjVO update(StatusCpfCnpjVO vo) {
		...
		try {
			...
			sqlQuery.append("  UPDATE " + C_PL_SCHEMA + "TPL_PRVT_CPF_STAT SET " );
			sqlQuery.append(" CPF_STATUS    = ?, ");
			sqlQuery.append(" CPF_UPD_MTH_YR    = ?, ");
			sqlQuery.append(" LAST_AUTH_DATE    = ?, ");
			sqlQuery.append(" LAST_AUTH_USER_ID = ?, ");
	...
	public void delete(String cpfCnpjNbr) {
		...
		try {
			...
			query.append(" DELETE FROM " + C_PL_SCHEMA + "TPL_PRVT_CPF_STAT WHERE CPF_CNPJ_NBR = ? ");
	...
	public void deleteByMonthYear(String monthYear) {
		...
		try {
			...
			query.append(" DELETE FROM " + C_PL_SCHEMA + "TPL_PRVT_CPF_STAT WHERE CPF_UPD_MTH_YR = ? ");
