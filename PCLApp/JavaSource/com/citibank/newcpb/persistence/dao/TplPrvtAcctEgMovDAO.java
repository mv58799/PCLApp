...
public class TplPrvtAcctEgMovDAO extends BaseOracleDAO {

	public AcctEgVO getAccountEg(String acctNbr) {
		...
		try {
			...
			query.append(" SELECT TPL_PRVT_ACCT_EG_MOV.ER_NBR, "
			+ "   TPL_PRVT_ACCT_EG_MOV.ACCT_NBR, "
			+ "   TPL_PRVT_ACCT_EG_MOV.EG_NBR, "
			+ "   TPL_PRVT_ACCT_EG_MOV.LAST_AUTH_DATE, "
			+ "   TPL_PRVT_ACCT_EG_MOV.LAST_AUTH_USER_ID, "
			+ "   TPL_PRVT_ACCT_EG_MOV.LAST_UPD_DATE, "
			+ "   TPL_PRVT_ACCT_EG_MOV.LAST_UPD_USER_ID, "
			+ "   TPL_PRVT_ACCT_EG_MOV.REC_STAT_CODE"
			+ " FROM " + C_PL_SCHEMA + "TPL_PRVT_ACCT_EG_MOV "
			+ " WHERE TPL_PRVT_ACCT_EG_MOV.ACCT_NBR = TRIM(?) ");
		...
		try {
			...
			if(filter!=null){
				if (!StringUtils.isBlank(filter.getErNbr())) {
					query.append(" AND UPPER(TPL_PRVT_ACCT_EG_MOV.ER_NBR) = TRIM(?) ");
				}
				
				if (!StringUtils.isBlank(filter.getAcctNbr())) {
					query.append(" AND UPPER(TPL_PRVT_ACCT_EG_MOV.ACCT_NBR) = TRIM(?) ");
				}
				
				if (!StringUtils.isBlank(filter.getEgNbr())) {
					query.append(" AND UPPER(TPL_PRVT_ACCT_EG_MOV.EG_NBR) = TRIM(?) ");
				}
				
				if (!StringUtils.isBlank(filter.getRecStatCode())) {
					query.append(" AND UPPER(TPL_PRVT_ACCT_EG_MOV.REC_STAT_CODE) = TRIM(?) ");
				}
				
				if (notInative!=null && notInative) {
					query.append(" AND TPL_PRVT_ACCT_EG_MOV.REC_STAT_CODE <> 'I' ");
				}
			}
	...
	public void insert(AcctEgVO vo) {
		...
		try {
			...
			sqlQuery.append("  INSERT INTO " + C_PL_SCHEMA + "TPL_PRVT_ACCT_EG_MOV ( " );
	...
	public void delete(String acctNbr) {
		...
		try {
			...
			query.append(" DELETE FROM " + C_PL_SCHEMA + "TPL_PRVT_ACCT_EG_MOV WHERE ACCT_NBR = ? ");
