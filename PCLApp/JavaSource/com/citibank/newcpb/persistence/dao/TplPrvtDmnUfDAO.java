...
public class TplPrvtDmnUfDAO extends BaseOracleDAO {
	...
	public ArrayList<ResultTableBean> list() {
		...
		try {
			...
			query.append("SELECT UF_CODE, UF_TEXT "
			+ "FROM " + C_PL_SCHEMA + "TPL_PRVT_DMN_UF ORDER BY UF_TEXT");
			...
			if(rs!=null){
				...
				while (rs.next()){
					...
					result.setResultDescription(rs.getString("UF_TEXT") != null ? rs.getString("UF_TEXT").toString() : null);								
					result.setResultCode(rs.getString("UF_CODE") != null ? rs.getString("UF_CODE").toString() : null);
