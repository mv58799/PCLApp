...
import com.citibank.newcpb.enun.CpfCnpjStatusEnum;
...
public class StatusCpfCnpjAction extends CommonAction {
	
	private StatusCpfCnpjService service;
	private RegisterDataCustomerService custService;

	public StatusCpfCnpjAction() {
		service = new StatusCpfCnpjService();
	...
	public final ActionForward execute(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			StatusCpfCnpjForm formLocal = (StatusCpfCnpjForm) actionForm;
			...
			try {
				...
				if (!hasDataFormErrors) {
					...
					if(StringUtils.isBlank(method)){
						formLocal = new StatusCpfCnpjForm();
						...
						if(StringUtils.isBlank(formLocal.getFilterNumberEM()) && StringUtils.isBlank(formLocal.getFilterCpfCnpj()) &&
								StringUtils.isBlank(formLocal.getFilterName()) && StringUtils.isBlank(formLocal.getFilterStatus())
								 && StringUtils.isBlank(formLocal.getFilterMonthYear())){
							
							formLocal.addError(StatusCpfCnpjForm.C_ERROR_EMPTY_FILTER);
							formLocal.setResultList(new ArrayList<StatusCpfCnpjVO>());
							
						}else{
							
							ArrayList<StatusCpfCnpjVO> resultList = service.listCad(formLocal.getFilterNumberEM(), formLocal.getFilterCpfCnpj(), formLocal.getFilterName(), formLocal.getFilterStatus(), formLocal.getFilterMonthYear(), null, true);				
						...
						if(formLocal.getLoggedUser()!=null && formLocal.getLoggedUser().getUserAccess()!=null 
							...
							formLocal.setStatusValues(CpfCnpjStatusEnum.getTypeList());
				...
				if (formLocal != null) {					
					...
					saveMessages(request, cloneMessage(formLocal));
					saveErrors(request, (ActionMessages) cloneErrors(formLocal));
					request.getSession().setAttribute("StatusCpfCnpjForm", formLocal);
				}
	...
	public StatusCpfCnpjService getService() {
		return service;
	}

	public void setService(StatusCpfCnpjService service) {
